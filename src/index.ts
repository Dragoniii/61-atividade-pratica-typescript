...
const nota1: number = Number(((Math.random() * 9) + 1.01).toFixed(2));
const nota2: number = Number(((Math.random() * 9) + 1.01).toFixed(2));

const media: number = Number(((nota1 + nota2) / 2).toFixed(2));

const aprovado: boolean = media >= 6;

const Resposta1: { exercicio: number; nota1: number; nota2: number; media: number; aprovado: boolean } = { exercicio, nota1, nota2, media, aprovado };
